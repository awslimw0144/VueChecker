...
public class NACWeb_Esplanade_TestContent {
    ...
    public void user_Should_Be_Able_To_See_The_Matched_Content(){
        this.sTestIDs.forEach(sTestID -> {
            ...
            String sExpectedContent = nacDataBase.getValue("DataSet",sTestID,"content");
            ...
            then(userTaiger).should(
                    seeThat("the title retrieved from response matches ",
                            VerifyNAC_Esplanade.content(sExpectedContent),equalTo(true)
            ));
        });
    }
}