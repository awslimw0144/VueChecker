...
public class NACWeb_Esplanade_TestLanguage {
    private TreeSet<String> sTestIDs;
    private final ExcelTestable nacDataBase = new NAC_DataBase(
            ResrcUtils.getFilePath(Application_ExcelObject.class,
                    "Nac-database-20200908-Sep-Accuracy-Report2.xlsx"));
    ...
    public void user_Should_Be_Able_To_See_The_Matched_Content(){
        this.sTestIDs.forEach(sTestID -> {
            ...
            String sExpectedLanguage = nacDataBase.getValue("DataSet",sTestID,"tags.language");
            ...
            then(userTaiger).should(
                    seeThat("the title retrieved from response matches ",
                            VerifyNAC_Esplanade.language(sExpectedLanguage),equalTo(sExpectedLanguage)
            ));
        });
    }
}