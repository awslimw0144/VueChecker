...
public class NACWeb_Esplanade_TestMetaPrice {
    ...
    @Test
    public void user_Should_Be_Able_To_See_The_Matched_Meta_Price(){
        this.sTestIDs.forEach(sTestID -> {
            LoggerUtils.printInfo("=============  TEST ["+sTestID+"] =========================");
            String sPath = nacDataBase.getValue("DataSet",sTestID,"path");
            String sMetaPrice = nacDataBase.getValue("DataSet",sTestID,"metadata.price");
            ...
            then(userTaiger).should(
                    seeThat("the price in comparison with meta price", VerifyNAC_Esplanade.metaPrice(),
                            equalTo(sMetaPrice)));
        });
    }
}