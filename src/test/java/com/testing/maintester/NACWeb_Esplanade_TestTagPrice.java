...
public class NACWeb_Esplanade_TestTagPrice {
    private TreeSet<String> sTestIDs;
    private final ExcelTestable nacDataBase = new NAC_DataBase(ResrcUtils
            .getFilePath(Application_ExcelObject.class,"Nac-database-20200908-Sep-Accuracy-Report2.xlsx"));
    ...
    @Test
    public void user_Should_Be_Able_To_See_The_Matched_Tag_Price(){
        this.sTestIDs.forEach(sTestID -> {
            LoggerUtils.printInfo("=============  TEST ["+sTestID+"] =========================");
            String sPath = nacDataBase.getValue("DataSet",sTestID,"path");
            String sTagPrice = nacDataBase.getValue("DataSet",sTestID,"tags.price");
            ...
            then(userTaiger).should(
                    seeThat("the price retrieved from web falls into the logic defined by tag.price ",
                            VerifyNAC_Esplanade.tagPriceMatches(Arrays.stream(sTagPrice.split(Pattern.quote(",")))
