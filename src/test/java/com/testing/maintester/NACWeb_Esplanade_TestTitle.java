...
public class NACWeb_Esplanade_TestTitle {
    ...
    public void user_Should_Be_Able_To_See_The_Matched_Title(){
        this.sTestIDs.forEach(sTestID -> {
            ...
            String sExpectedTitle = nacDataBase.getValue("DataSet",sTestID,"title");
            ...
            then(userTaiger).should(
                    seeThat("the title retrieved from response matches ", VerifyNAC_Esplanade.title(),equalTo(sExpectedTitle))
            );
        });
    }
}